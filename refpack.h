#ifndef REFPACK_CPP_REFPACK_H
#define REFPACK_CPP_REFPACK_H
...
inline uint32_t HASH(const uint8_t *s) {
    ...
    crc = crctab[static_cast<uint8_t>(s[0])];
    crc = crctab[static_cast<uint8_t>(crc ^ s[1])] ^ (crc >> 8);
    crc = crctab[static_cast<uint8_t>(crc ^ s[2])] ^ (crc >> 8);

    return crc;
}

#endif //REFPACK_CPP_REFPACK_H
